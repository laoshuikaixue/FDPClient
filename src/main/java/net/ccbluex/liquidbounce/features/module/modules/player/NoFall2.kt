...
@ModuleInfo(name = "NoFall2", category = ModuleCategory.PLAYER)
class NoFall2 : Module() {
    private val typeValue = ListValue("Type", arrayOf("Edit", "Packet", "MLG", "AAC", "Spartan", "CubeCraft", "Hypixel", "Phase", "Verus", "Medusa", "Motion", "Matrix"), "Edit")

    private val editMode = ListValue("Edit-Mode", arrayOf("Always", "Default", "Smart", "NoGround", "Damage"), "Always").displayable { typeValue.get().equals("edit", true) }
    private val packetMode = ListValue("Packet-Mode", arrayOf("Default", "Smart"), "Default").displayable { typeValue.get().equals("packet", true) }
    private val aacMode = ListValue("AAC-Mode", arrayOf("Default", "LAAC", "3.3.11", "3.3.15", "4.x", "4.4.x", "Loyisa4.4.2", "5.0.4", "5.0.14"), "Default").displayable { typeValue.get().equals("aac", true) }
    private val hypixelMode = ListValue("Hypixel-Mode", arrayOf("Default", "Packet", "New"), "Default").displayable { typeValue.get().equals("hypixel", true) }
    private val matrixMode = ListValue("Matrix-Mode", arrayOf("Old", "6.2.x", "6.6.3"), "Old").displayable { typeValue.get().equals("matrix", true) }

    private val phaseOffsetValue = IntegerValue("PhaseOffset", 1, 0, 5).displayable { typeValue.get().equals("phase", true) }
    private val minFallDistanceValue = FloatValue("MinMLGHeight", 5F, 2F, 50F).displayable { typeValue.get().equals("mlg", true) }
    private val flySpeedValue = FloatValue("MotionSpeed", -0.01F, -5F, 5F).displayable { typeValue.get().equals("motion", true) }
