...
@ModuleInfo(
    name = "WallClimb", category = ModuleCategory.MOVEMENT)
class WallClimb : Module() {
    private val modeValue =
        ListValue("Mode", arrayOf("Simple", "CheckerClimb", "Clip", "AAC3.3.12", "AACGlide", "Verus"), "Simple")
    private val clipMode =
        ListValue("ClipMode", arrayOf("Jump", "Fast"), "Fast").displayable { modeValue.equals("clip") }
    private val checkerClimbMotionValue =
        FloatValue("CheckerClimbMotion", 0f, 0f, 1f).displayable { modeValue.equals("checkerclimb") }
    private val verusClimbSpeed =
        FloatValue("VerusClimbSpeed", 0f, 0f, 1f).displayable { modeValue.equals("verus") }
