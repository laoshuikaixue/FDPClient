...
class NewBlocksMcFly : FlyMode("BlocksMc") {
    private val speedValue = FloatValue("${valuePrefix}Speed", 2f, 0f, 5f)
    private val kickBypassValue = BoolValue("${valuePrefix}KickBypass", true)
    ...
    private val spoofValue = BoolValue("${valuePrefix}SpoofGround", true)
    ...
    override fun onPacket(event: PacketEvent) {
        ...
        if (packet is C03PacketPlayer) {
            ...
            if (packets == 40 && kickBypassValue.get()) {
                ...
                LiquidBounce.hud.addNotification(Notification("BlocksMcFly", "VanillaKickBypass.", NotifyType.SUCCESS, 1000))
                packets = 0
            }
        }
    }
