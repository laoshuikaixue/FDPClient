...
public class NullStyle2 extends Style {
    ...
    public void drawPanel(int mouseX, int mouseY, Panel panel) {
        RenderUtils.drawGradientSideways((float) panel.getX() - 3, (float) panel.getY(), (float) panel.getX() + panel.getWidth() + 3, (float) panel.getY() + 19, modifyAlpha(ClickGUIModule.generateColor(), 120).getRGB(), modifyAlpha(ClickGUIModule.generateColor().darker().darker(), 120).getRGB());
    ...
    public void drawDescription(int mouseX, int mouseY, String text) {
        ...
        RenderUtils.drawRect(mouseX + 9, mouseY, mouseX + textWidth + 14, mouseY + Fonts.font35.FONT_HEIGHT + 3, modifyAlpha(ClickGUIModule.generateColor(), 120).getRGB());
        GlStateManager.resetColor();
        ...
        if(!moduleValues.isEmpty()) {
            ...
            if(moduleElement.isShowSettings()) {
                ...
                for(final Value value : moduleValues) {
                    if (!((boolean)value.getDisplayable()))
                        continue;
