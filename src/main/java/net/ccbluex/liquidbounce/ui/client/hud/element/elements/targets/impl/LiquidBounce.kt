...
class LiquidBounce(inst: Target2): TargetStyle("LiquidBounce", inst) {

    val hurtTimeAnim = BoolValue("HurtTimeAnim", true).displayable { targetInstance.styleValue.get().equals("liquidbounce", true) }
    val borderColorMode = ListValue("Border-Color", arrayOf("Custom", "MatchBar", "None"), "None").displayable { targetInstance.styleValue.get().equals("liquidbounce", true) }
    val borderWidthValue = FloatValue("Border-Width", 3F, 0.5F, 5F).displayable { targetInstance.styleValue.get().equals("liquidbounce", true) }
    val borderRedValue = IntegerValue("Border-Red", 0, 0, 255).displayable { targetInstance.styleValue.get().equals("liquidbounce", true) && borderColorMode.get().equals("custom", true) }
    val borderGreenValue = IntegerValue("Border-Green", 0, 0, 255).displayable { targetInstance.styleValue.get().equals("liquidbounce", true) && borderColorMode.get().equals("custom", true) }
    val borderBlueValue = IntegerValue("Border-Blue", 0, 0, 255).displayable { targetInstance.styleValue.get().equals("liquidbounce", true) && borderColorMode.get().equals("custom", true) }
    val borderAlphaValue = IntegerValue("Border-Alpha", 0, 0, 255).displayable { targetInstance.styleValue.get().equals("liquidbounce", true) && borderColorMode.get().equals("custom", true) }
